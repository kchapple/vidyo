...
$startRecordingParam = array();

$conferenceID = $_GET['conferenceID'];
$startRecordingParam['conferenceID'] = $conferenceID;
$recorderPrefix = $_GET['recorderPrefix'];
$startRecordingParam['recorderPrefix'] = $recorderPrefix;

if( $webcast = $_GET['webcast'] )
	$webcast = true;
else
	$webcast = false;
$startRecordingParam['webcast'] = $webcast;

if( $moderatorPIN = $_GET['moderatorPIN'] )
	$startRecordingParam['moderatorPIN'] = $moderatorPIN;

try {
	$startRecordingResult = $client->startRecording($startRecordingParam);
	if ($startRecordingResult->OK)
	    echo "Recording started successfully<br><br>";
