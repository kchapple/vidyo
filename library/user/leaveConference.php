...
// prepare leave conference
if( $conferenceID = $_GET['conferenceID'] ) {
	if( $participantID = $_GET['participantID'] ) {
		$leaveConfParam = array('conferenceID' => $conferenceID, 'participantID' => $participantID);
		try {
			$leaveConfResult = $client->leaveConference($leaveConfParam);
			$leaveConfResult = $leaveConfResult->OK;
			echo "Leave Conf result is $leaveConfResult<br><br>";
		} catch (SoapFault $e){
