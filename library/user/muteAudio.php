...
if( $conferenceID = $_GET['conferenceID'] ) {
	if( $participantID = $_GET['participantID'] ) {
        $muteAudioParam = array('conferenceID' => $conferenceID, 'participantID' => $participantID);
        try {
            $muteAudioResult = $client->muteAudio($muteAudioParam);
            $muteAudioResult = $muteAudioResult->OK;
            echo "MuteAudio result is $muteAudioResult<br><br>";
        } catch (SoapFault $e){
