...
if( $conferenceID = $_GET['conferenceID'] ) {
	if( $participantID = $_GET['participantID'] ) {
        $startVideoParam = array('conferenceID' => $conferenceID, 'participantID' => $participantID);
        try {
            $startVideoResult = $client->startVideo($startVideoParam);
            $startVideoResult = $startVideoResult->OK;
            echo "startVideo result is $startVideoResult<br><br>";
        } catch (SoapFault $e){
