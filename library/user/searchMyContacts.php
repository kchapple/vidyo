...
if ($useAuthToken=$_GET['useAuthToken']){
    if ($authToken){
        echo "Using Authentication token<br>$authToken<br>";
        ...
        echo "<b>No authToken defined<br>Using Username/Password</b><br>";
        $client = new SoapClient("http://$vidyoportal/services/v1_1/VidyoPortalUserService?wsdl", 
    ...
    echo "Using Username/Password<br>";
    $client = new SoapClient("http://$vidyoportal/services/v1_1/VidyoPortalUserService?wsdl", 
                             array('login' => $username,
                                   'password' => $password,
                                   'trace' => 1, 
...
$searchMyContactsParam = array('Filter' => $filter);
$c = 1;
try {
	$searchResult = $client->searchMyContacts($searchMyContactsParam);
