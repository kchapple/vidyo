...
// prepare unmute audio
if( $conferenceID = $_GET['conferenceID'] ) {
	if( $participantID = $_GET['participantID'] ) {
        $unmuteAudioParam = array('conferenceID' => $conferenceID, 'participantID' => $participantID);
        try {
            $unmuteAudioResult = $client->unmuteAudio($unmuteAudioParam);
            $unmuteAudioResult = $unmuteAudioResult->OK;
            echo "unmuteAudio result is $unmuteAudioResult<br><br>";
        } catch (SoapFault $e){
